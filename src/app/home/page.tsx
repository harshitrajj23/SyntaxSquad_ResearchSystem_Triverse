...
import {
  Cpu, Activity, User, Settings, LogOut, MessageSquare,
...
export default function HomePage() {
  ...
  const quickActions = [
    {
      title: "Neural Chat",
      desc: "Start a conversation with your AI",
      icon: <MessageSquare className="w-6 h-6" />,
      href: "/chat/new",
      gradient: "from-[#00AEEF] to-[#00F0FF]",
      glow: "shadow-[0_0_40px_rgba(0,174,239,0.3)]",
      primary: true
    },
    {
      title: "Chat History",
      ...
      href: "/history",
      gradient: "from-[#9d00ff] to-[#FF006E]",
      glow: "shadow-[0_0_40px_rgba(157,0,255,0.2)]",
      primary: false
    },
    {
      title: "Evolution Logs",
      desc: "Track AI adaptation progress",
      icon: <BarChart3 className="w-6 h-6" />,
      href: "#",
      gradient: "from-[#00FF94] to-[#00AEEF]",
      glow: "shadow-[0_0_40px_rgba(0,255,148,0.2)]",
      primary: false
    }
  ];
  ...
  return (
    <div className="min-h-screen bg-black text-white relative">
      ...
      <div className="fixed top-[-20%] left-[-10%] w-[60vw] h-[60vw] bg-[#00AEEF]/[0.03] blur-[150px] rounded-full pointer-events-none" />
      <div className="fixed bottom-[-20%] right-[-10%] w-[50vw] h-[50vw] bg-[#9d00ff]/[0.03] blur-[150px] rounded-full pointer-events-none" />
      ...
      <header className="sticky top-0 z-50 border-b border-white/[0.06] bg-black/60 backdrop-blur-2xl">
        <div className="max-w-[1400px] mx-auto px-6 md:px-10 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="font-black text-lg tracking-tight">THINK LOOP</span>
            <span className="hidden md:inline-block text-[9px] text-white/20 bg-white/5 px-2 py-0.5 rounded-full font-bold tracking-widest border border-white/5 ml-2">v2.0 BETA</span>
          </div>

          <nav className="flex items-center gap-2 md:gap-3">
            <Link
              href="/chat/new"
              className="flex items-center gap-2 bg-gradient-to-r from-[#00AEEF] to-[#00F0FF] text-black px-5 py-2.5 rounded-xl font-bold text-[11px] tracking-widest uppercase hover:shadow-[0_0_30px_rgba(0,174,239,0.5)] hover:scale-[1.02] transition-all duration-300"
            >
            ...
            <Link
              href="/history"
              className="flex items-center gap-2 bg-white/[0.04] border border-white/[0.08] text-white/60 px-4 py-2.5 rounded-xl hover:bg-white/[0.08] hover:text-white transition-all text-[11px] font-bold tracking-widest uppercase"
            >
            ...
            <button
              ...
              className="flex items-center gap-2 bg-white/[0.04] border border-white/[0.08] px-4 py-2.5 rounded-xl hover:bg-red-500/10 hover:border-red-500/20 hover:text-red-400 transition-all text-[11px] font-bold tracking-widest uppercase text-white/60"
            >
      ...
      <main className="max-w-[1400px] mx-auto px-6 md:px-10 py-10 relative z-10">
        ...
        >
          <motion.div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-2" variants={fadeUp} custom={0}>
            ...
            <motion.div variants={fadeUp} custom={3} className="flex-shrink-0">
              <Link
                href="/chat/new"
                className="group relative inline-flex items-center gap-3 bg-gradient-to-r from-[#00AEEF] to-[#00F0FF] text-black px-8 py-4 rounded-2xl font-bold text-sm tracking-wide hover:shadow-[0_0_50px_rgba(0,174,239,0.5)] hover:scale-[1.03] transition-all duration-300"
              >
        ...
        <motion.section className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10" initial="hidden" animate="visible">
          {stats.map((stat, i) => (
            <motion.div
              ...
              variants={scaleIn}
              custom={i}
              className="glass-card rounded-2xl p-5 md:p-6 relative group hover:border-white/[0.12] transition-all duration-500 overflow-hidden cursor-default"
            >
              {/* Glow effect */}
              <div className={`absolute -top-10 -right-10 w-24 h-24 ${stat.bgGlow} opacity-[0.06] blur-[40px] rounded-full group-hover:opacity-[0.12] transition-opacity duration-700`} />
              ...
              <div className="flex items-end gap-2">
                ...
                <span className={`text-[10px] font-bold mb-1 ${stat.change.startsWith('+') || stat.change.startsWith('-') ? (stat.change.includes('-') && stat.label !== 'Accuracy' ? 'text-[#00FF94]' : 'text-[#00FF94]') : 'text-white/40'}`}>
                  {stat.change}
                </span>
              </div>
              ...
              <div className="flex items-end gap-[3px] mt-4 h-6">
                {[40, 65, 45, 80, 55, 90, 70, 85, 95, 60, 75, 88].map((h, j) => (
                  <div
                    key={j}
                    className={`w-[4px] rounded-full bg-gradient-to-t ${stat.color} opacity-20 group-hover:opacity-40 transition-all duration-500`}
                    style={{
                      height: `${h}%`,
          ...
          >
            {quickActions.map((action, i) => (
              <motion.div key={action.title} variants={fadeUp} custom={i + 4}>
                <Link
                  href={action.href}
                  className={`block glass-card rounded-2xl p-6 md:p-8 relative group overflow-hidden transition-all duration-500 h-full ${action.primary ? 'border-[#00AEEF]/20 hover:border-[#00AEEF]/40' : 'hover:border-white/[0.12]'} ${action.glow.replace('shadow', 'hover:shadow')}`}
                >
                  {/* Background gradient blob */}
                  <div className={`absolute -bottom-10 -right-10 w-32 h-32 bg-gradient-to-br ${action.gradient} opacity-[0.06] blur-[50px] rounded-full group-hover:opacity-[0.15] group-hover:scale-150 transition-all duration-700`} />
        ...
        {/* Evolution Progress */}
        <motion.section
          className="glass-card rounded-2xl p-8 md:p-10 relative overflow-hidden mb-10"
          initial="hidden"
          animate="visible"
          variants={fadeUp}
          custom={8}
          ...
          <div className="relative z-10">
            ...
            <div className="relative">
              <div className="w-full h-2 bg-white/[0.06] rounded-full overflow-hidden">
                <motion.div
                  className="h-full bg-gradient-to-r from-[#00AEEF] to-[#00F0FF] rounded-full relative"
                  initial={{ width: 0 }}
                  animate={{ width: "33%" }}
                  ...
                  <div key={i} className="flex flex-col items-center gap-2">
                    <div className={`w-3 h-3 rounded-full border-2 ${
                      milestone.status === "done"
                        ? "bg-[#00AEEF] border-[#00AEEF] shadow-[0_0_10px_rgba(0,174,239,0.5)]"
                        : milestone.status === "active"
                        ? "bg-transparent border-[#00F0FF] shadow-[0_0_10px_rgba(0,240,255,0.3)] animate-pulse"
                        : "bg-transparent border-white/10"
                    }`} />
                    <span className={`text-[9px] font-bold tracking-widest uppercase ${
                      milestone.status === "active" ? "text-neon-blue" : milestone.status === "done" ? "text-white/50" : "text-white/20"
                    }`}>
                      {milestone.label}
                    </span>
                  </div>
